...
import { CheckIcon, ChevronRightIcon, DotFilledIcon } from '@paalan/react-icons';
import { cn } from '@paalan/react-shared/lib';
import * as DropdownMenuPrimitive from '@radix-ui/react-dropdown-menu';

import type { CustomDropdownMenuItem, CustomDropdownMenuItemWithSubItem, DropdownMenuProps } from './types';
...
const LocalDropdownMenuItem: React.FC<Omit<CustomDropdownMenuItem, 'subItems'>> = ({
  separator,
  label,
  shortcut,
  ...props
...
const isSubItem = (item: unknown): item is CustomDropdownMenuItemWithSubItem => {
  const subItem = item as CustomDropdownMenuItemWithSubItem;
  return Boolean(subItem.subItems) && !!subItem.subItems?.length;
};
...
const DropdownMenu: React.FC<DropdownMenuProps> = ({
  ...
  return (
    <DropdownMenuRoot {...props}>
      ...
      <DropdownMenuContent {...contentProps} className={cn('min-w-32', contentProps?.className)}>
        {title && (
          <>
        ...
        <DropdownMenuGroup>
          {items.map((item, index) => {
            if (!isSubItem(item)) {
              return <LocalDropdownMenuItem key={index} {...item} />;
            }
            return (
                ...
                <DropdownMenuPortal>
                  <DropdownMenuSubContent>
                    {item.subItems.map((subItem, subIndex) => (
                      <LocalDropdownMenuItem key={subIndex} {...subItem} />
                    ))}
                  </DropdownMenuSubContent>
                </DropdownMenuPortal>
              </DropdownMenuSub>
